...
export default {
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        // 確保 inventory 物件存在，避免讀取時發生 undefined 錯誤
        ...
        if (cloverCount > 0) {
            ...
            if (userData.inventory["lucky_clover"] <= 0) {
                delete userData.inventory["lucky_clover"];
            }
            cloverMessage = `\n🍀 **已消耗幸運草：** 你的勝率提升了！`;
            usedClover = true;
        } 
        // 2. 如果沒有幸運草，接著檢查幸運護符
            ...
            const remainingCharm = userData.inventory["lucky_charm"];
            
            if (remainingCharm <= 0) {
                delete userData.inventory["lucky_charm"];
            }
            
            cloverMessage = `\n🍀 **已使用幸運護符（剩餘 ${Math.max(0, remainingCharm)} 次）：** 你的勝率提升了！`;
            usedCharm = true;
        }
        ...
        if (win) {
            ...
            resultEmbed = warningEmbed(
                "💔 你輸了...",
                `骰子對你不利。你輸掉了 **$${betAmount.toLocaleString()}** 的下注。${cloverMessage}`,
            );
        }
        ...
        if (usedClover) {
            const leftClover = userData.inventory["lucky_clover"] || 0;
            resultEmbed.setFooter({
                text: `你還剩 ${leftClover} 個幸運草。當前勝率為 ${Math.round(winChance * 100)}%。`,
            });
        } else if (usedCharm) {
            const leftCharm = userData.inventory["lucky_charm"] || 0;
            resultEmbed.setFooter({
                text: `你還剩 ${leftCharm} 次幸運護符使用機會。當前勝率為 ${Math.round(winChance * 100)}%。`,
            });
        } else {
            resultEmbed.setFooter({
